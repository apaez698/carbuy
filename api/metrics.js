    ...
    switch (q) {
      ...
      case 'kpis': {
        const [sessions, leads, whaClicks] = await Promise.all([
          ...
          sb.from('events').select('*', { count: 'exact', head: true }).eq('event_type', 'whatsapp_click')
        ]);

        const today = startOfDay(0);
        const yesterday = startOfDay(1);

        const [leadsHoy, leadsAyer, sesHoy, sesAyer] = await Promise.all([
          sb.from('leads').select('*', { count: 'exact', head: true }).gte('created_at', today),
          sb.from('leads').select('*', { count: 'exact', head: true }).gte('created_at', yesterday).lt('created_at', today),
          sb.from('sessions').select('*', { count: 'exact', head: true }).gte('created_at', today),
          sb.from('sessions').select('*', { count: 'exact', head: true }).gte('created_at', yesterday).lt('created_at', today)
        ]);
      ...
      case 'funnel': {
        const { data } = await sb.from('events')
          ...
          .in('event_type', ['step_start', 'step_complete', 'form_submit', 'whatsapp_click']);
        return res.status(200).json({ data: data || [] });
      }
      ...
      // ── Últimos leads (tabla) ──
      case 'leads_tabla': {
        const { data, count } = await sb.from('leads')
          .select('id, created_at, nombre, marca, modelo, anio, kilometraje, estado_general, estimado_texto, celular, ciudad, utm_source', { count: 'exact' })
